...
qqplot.pvalue <- function(x){
  ...
  if(is.list(x)){
    ...
    ggplot(data = qq.melt, aes(expected, Observed,colour = label) )+geom_point(size = 1.5)+geom_abline(slope = 1)+xlab("Expected(-log10 p-value)")+ylab("Observed(-log10 p-value)")+
